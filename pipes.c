...
void child_and_operations (int fd_1, int fd_2, int* pipefds, int nForks, int num);
...
int main(int argc, char* argv[])
{
	...
	pid = fork();
    	if (!pid) {
		child_and_operations(input_d, pipefds[1], pipefds, nForks, 0);    	
	}
	
    	for (i = 1; i < nForks; i++) {
        	pid = fork();
        	if (!pid) 
			child_and_operations(pipefds[4 * i - 2], pipefds[4 * i + 1], pipefds, nForks, i);	
	}
